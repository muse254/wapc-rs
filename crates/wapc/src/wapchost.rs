...
use self::traits::{WebAssemblyEngineProvider, WebAssemblyEngineProviderAsync};
...
/// An asynchronous version of the `WapcHost` struct.
/// There is effectively no difference between the `WapcHost` and `WapcHostAsync`, other than
/// this is for use in caller async contexts but the underlying engine provider is synchronous.
/// 
/// Async synchronization is left yo the caller to ensure that the `WapcHostAsync` is not accessed
/// concurrently from multiple threads.
pub struct WapcHostAsync {
  engine: RefCell<Box<dyn WebAssemblyEngineProviderAsync>>,
  state: Arc<ModuleState>,
}
...
impl WapcHostAsync {
  ...
  /// low-level engine provider
  pub fn new(
    engine: Box<dyn WebAssemblyEngineProviderAsync>,
